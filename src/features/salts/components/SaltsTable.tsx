// path: frontend/src/features/salts/components/SaltsTable.tsx
...
interface Props {
  rows: Salt[];
  onEdit: (salt: Salt) => void;
}

const ROW_HEIGHT = 36;

export function SaltsTable({ rows, onEdit }: Props) {
  const columns = useMemo<ColumnDef<Salt>[]>(
    () => [
      { accessorKey: "name", header: "Name", size: 300 },
      {
        accessorKey: "defaultStrength",
        header: "Strength",
        size: 200,
        cell: (c) => (

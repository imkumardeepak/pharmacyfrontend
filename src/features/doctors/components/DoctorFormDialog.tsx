// path: frontend/src/features/doctors/components/DoctorFormDialog.tsx
import {
  useEffect,
  useRef,
  useState,
...
import type { Doctor, DoctorInput } from "../api/doctors.api";
...
export function DoctorFormDialog({ open, initial, onClose }: Props) {
  ...
  return (
    <Dialog
      open={open}
      onClose={onClose}
      title={isEdit ? `Edit Doctor — ${initial?.name}` : "New Doctor"}
    >
      <form onSubmit={onSubmit} className="grid grid-cols-2 gap-4">
        ...
        <Field label="Qualification">
          <Input
            ...
            onChange={(e) =>
              setForm({ ...form, qualification: e.target.value || null })
            }
            onKeyDown={onFieldKeyDown}
            maxLength={100}
          />
        ...
        <Field label="Reg Number">
          <Input
            ...
            onChange={(e) =>
              setForm({ ...form, regNumber: e.target.value || null })
            }
            onKeyDown={onFieldKeyDown}
            maxLength={50}
          />
